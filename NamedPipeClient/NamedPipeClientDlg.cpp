...
DWORD WINAPI Th(LPVOID p)
{
	...
	for (;;)
	{
		if (WaitNamedPipe(_T("\\\\.\\pipe\\mypipe"), NMPWAIT_WAIT_FOREVER) == TRUE)
		{
			hPipe = CreateFile(_T("\\\\.\\pipe\\mypipe"), GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
			AfxMessageBox(_T("대기 성공"));
...
void CNamedPipeClientDlg::OnBnClickedButton1()
{
	...
	CloseHandle(CreateThread(0, 0, Th, this, 0, 0));
}
